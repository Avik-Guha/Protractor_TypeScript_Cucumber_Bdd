 ...
 * @package
 */
class SelectedValue extends core_1.Question {
    constructor(target) {
        super(io_1.formatted `value selected in ${target}`);
        this.target = target;
    }
    answeredBy(actor) {
        const value = withAnswerOf_1.withAnswerOf(actor, this.target, (element) => Value_1.Value.of(element.$('option:checked')).answeredBy(actor));
        return promiseOf_1.promiseOf(value);
    }
}
/**
 * @package
 */
class SelectedValues extends core_1.Question {
    constructor(target) {
        super(io_1.formatted `values selected in ${target}`);
        this.target = target;
    }
    answeredBy(actor) {
        const options = withAnswerOf_1.withAnswerOf(actor, this.target, (element) => element.$$('option')
            .filter(option => option.isSelected()));
        return promiseOf_1.promiseOf(options.map(option => Value_1.Value.of(option).answeredBy(actor)));
    }
}
/**
 * @package
 */
class SelectedOption extends core_1.Question {
    constructor(target) {
        super(io_1.formatted `option selected in ${target}`);
        this.target = target;
    }
    answeredBy(actor) {
        return promiseOf_1.promiseOf(withAnswerOf_1.withAnswerOf(actor, this.target, element => element.$('option:checked').getText()));
    }
}
/**
 * @package
 */
class SelectedOptions extends core_1.Question {
    constructor(target) {
        super(io_1.formatted `options selected in ${target}`);
        this.target = target;
    }
    answeredBy(actor) {
        return promiseOf_1.promiseOf(withAnswerOf_1.withAnswerOf(actor, this.target, element => element.$$('option')
            .filter(option => option.isSelected())
